...
class CurveDrawer {
	constructor()
	{
		...
		this.transform_values = gl.getAttribLocation(this.prog, "t");
	...
	setViewport( width, height )
	{
		...
		gl.uniformMatrix4fv(this.mvp,false,[ 2/width,0,0,0,  0,-2/height,0,0, 0,0,1,0, -1,1,0,1 ]);
	...
	draw()
	{
		...
		gl.enableVertexAttribArray( this.transform_values );
		gl.vertexAttribPointer( this.transform_values, 1, gl.FLOAT, false, 0, 0 );
...
var curvesVS = `
	...
	void main()
	{
		bCurveX = (pow(1.0 -t,3.0)*p0.x) + (3.0*pow(1.0-t,2.0)*t*p1.x) + (3.0*(1.0-t) * pow(t,2.0) * p2.x) + (pow(t,3.0) * p3.x);

		bCurveY = (pow(1.0 -t,3.0)*p0.y) + (3.0*pow(1.0-t,2.0)*t*p1.y) + (3.0*(1.0-t) * pow(t,2.0) * p2.y) + (pow(t,3.0) * p3.y);
		
		gl_Position =  mvp * vec4(bCurveX,bCurveY,0,1);
	}
`;
